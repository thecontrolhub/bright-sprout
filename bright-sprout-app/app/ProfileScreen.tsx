...
import { Rocket, Zap, Leaf, AlertCircle } from '@tamagui/lucide-icons';
...
const achievements: Achievement[] = [
  { title: 'Course Starter', icon: Rocket, color: '#58CC02' },
  { title: 'Quick Learner', icon: Zap, color: '#FFC107' },
  { title: 'Master of Photosynthesis', icon: Leaf, color: '#F44336' },
];
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        {displayProfile ? (
          <YStack space="$4">
            <YStack alignItems="center" space="$2">
              {displayProfile.avatar ? (
                ...
                <YStack width={150} height={150} borderRadius={75} backgroundColor="$success" justifyContent="center" alignItems="center">
          ...
          <YStack flex={1} justifyContent="center" alignItems="center" padding="$4" backgroundColor="$background" borderRadius="$4" marginTop="$6" shadowColor="#000" shadowOffset={{ width: 0, height: 2 }} shadowOpacity={0.1} shadowRadius={4} elevation={5}>
            <AlertCircle size={80} color="$gray8" />
            <Paragraph color="$color.secondary" marginBottom="$3">No user profile found.</Paragraph>
            <Button backgroundColor="$success" color="$color" fontWeight="bold" onPress={() => router.push('/(auth)/Login')}>
              Go to Login
            </Button>
          </YStack>
        )}
